...
Asteroid::Sidebar::Sidebar() {
    ...
    onWorldChange(world);
    world.addChangeCallback(std::bind(&Asteroid::Sidebar::onWorldChange, this, std::placeholders::_1));
}
...
void Asteroid::Sidebar::step(GameState state, double dt, double t) {}

void Asteroid::Sidebar::draw(GameState state) {
    auto world = World::getInstance();
...
void Asteroid::Sidebar::onWorldChange(World world) {
    glBindVertexArray(vao);
    glBindBuffer(GL_ARRAY_BUFFER, vbo);
